...
import { IUSBPrinter, USBPrinter } from "react-native-thermal-receipt-printer";



export default function Usb_Printer() {
  const [printers, setPrinters] = useState<IUSBPrinter[]>([]);
  const [currentPrinter, setCurrentPrinter] = useState<IUSBPrinter | undefined>(undefined);
  ...
  useEffect(() => {
    USBPrinter.init()
      ...
      .catch((error) => {
        console.warn("Erro ao inicializar USBPrinter:", error);
      });
  }, []);

  const connectPrinter = async (selectPrinter: IUSBPrinter) => {
    try {
      const printer = await USBPrinter.connectPrinter(selectPrinter.vendor_id, selectPrinter.product_id);
  ...
  const printTextTest = () => {
    ...
    USBPrinter.printText("<C>Teste de impressao</C>\n");
  };
      ...
      {printers.map((printer, index) => (
        ...
        >
          ...
          <Text style={styles.macText}>
            {printer.device_name}:{printer.product_id}
          </Text>
        </TouchableOpacity>
      ))}
